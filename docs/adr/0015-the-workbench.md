...
- **Project redefined:** a Project is a Workbench entry referencing **one or more** code repos (`repos:` map + `default_repo` in its CONFIG), no longer "a repo path". Issues gain optional `repo:` frontmatter (omitted = default). *(Refined by ADR-0017: **zero** or more — a Project may be created repo-less, with a **workspace root** where code will live, and repos created during the drain self-heal into the registry.)* **One issue targets exactly one repo** — cross-repo work is a `depends_on` chain with the interface contract stated in the upstream issue. A Run's worktree/branch/merge/verify mechanics stay per-repo.
...
- **Memory:** `CORE.md` (capped) is injected into every Worker prompt and Dispatcher seed; bare terminal sessions get it via one standing instruction in the global `~/.claude/CLAUDE.md` (resolve cwd via registry — zero trace in any repo). The Dispatcher appends drain summaries to `journal/`; a **weekly scheduled curator agent** promotes/prunes/dedupes, flags contradictions and secret-shaped content, and is autonomous on `topics/`/`journal/` but **never writes `CORE.md` directly** — it only ever writes `memory/CORE.proposed.md` (memory poisoning steers every future session; CORE changes are cheap to approve, expensive to miss). **The sign-off channel (issue 169):** Mission Control's Attention surface renders the proposal beside the current `CORE.md` and offers Accept/Dismiss; a human's **confirmed** Accept click — naming itself as sign-off — is the *only* code path that promotes the proposal into `CORE.md` (it then deletes `CORE.proposed.md`), and Dismiss just deletes the proposal, leaving `CORE.md` untouched. The rule stands: the curator agent never edits CORE.md; a human's confirmed action, whether a hand edit or a confirmed MC Accept, is what signs it off.
