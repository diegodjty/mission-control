...
describe('manual-only — guides batch: needs the live shell / a real claude session (declared, not silently skipped)', () => {
  ...
  it.skip('manual-only: the Continue rows\' ✕ → confirm strip → project gone from the list, open-in-a-Window refusal message (issue 92) — reason: renderer + live registry ownership; the registry rewrite, one boring commit, no-longer-resolves, and dir-untouched invariants are asserted in project-remove.e2e.test.ts', () => {});
