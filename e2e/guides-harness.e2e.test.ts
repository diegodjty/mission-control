...
describe('manual-only — guides batch: needs the live shell / a real claude session (declared, not silently skipped)', () => {
  ...
  it.skip('manual-only: the Planning view Pane — a real claude session writing docs while the preview follows, stage buttons typing /grill-with-docs, /to-prd, /to-issues through the typing gate — reason: real LLM session (the issue declares this half manual-only); the watched set + recency ordering are asserted in Scenario d', () => {});
