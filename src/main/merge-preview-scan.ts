 ...
 * recompute through the SHARED per-repo serializer). The suspension and git-floor
 * gates match the handler's exactly: a mid-merge repo (a verdict would predict a
 * press that can't happen) and a below-floor git (no `merge-tree`) both yield no
 * previews for that repo, leaving every other repo's badges untouched.
 */
...
export interface RepoPreviewScanDeps {
  ...
  /** Is this repo left MID-MERGE? If so its previews are suspended (ADR-0018). */
 ...
 * per-repo body exactly: scan branches, read the mid-merge flag, and — only when
 * previews are supported AND the repo is not mid-merge AND it has at least one
 * finished-unmerged candidate — read the coordinator against this repo's own
 * serializer key. Any of those gates failing yields `previews: []` for THIS repo
 * alone; it never reaches into another repo's cache.
 */
export async function scanRepoWithPreviews(
  ...
  // so the renderer can block a new drain/Run and offer Abort — and suspend this
  // repo's previews (a verdict would predict a press that can't happen).
  const midMerge = await deps.isMidMerge(repo);
  let previews: BranchPreview[] = [];
  if (deps.previewSupported && !midMerge) {
